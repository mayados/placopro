...
import { createQuoteUseCase } from "@/modules/quote/use-cases/createQuoteUseCase";
...
export async function POST(req: NextRequest) {
  ...
  // const newQuote = await createQuoteUseCase({
  //   sanitizedData,
  //   userId: user.id,
  //   status,
  // });

    // Prisma transaction , and inject tx into use case
    const newQuote = await db.$transaction(async (tx) => {
      return await createQuoteUseCase({
        tx,
        sanitizedData,
        userId: user.id,
        status,
