...
import { deleteClient, fetchProspects } from "@/services/api/clientService";
    ...
    />
            <section className="flex-[8] px-4 py-6 bg-[#F5F5F5] rounded-md shadow-sm">
                <header className="mb-6">
                    <h1 className="text-3xl font-bold text-[#1873BF] text-center mb-2">Prospects</h1>
                </header>
                <SearchBar
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    onClear={() => setSearch("")}
                    placeholder="Rechercher un prospect (nom, ville, numéro client...)"
                />
